...
GPS::GPS(uart_inst_t* uart_hw):
    ...
    // 割り込み処理で実行する関数をセット
    irq_set_exclusive_handler(uart_irq, (_uart_hw==uart1) ? gps::read_raw1 : gps::read_raw0);
    // 割り込み処理を有効にする
...
GPS::Measurement_t GPS::read(){
    ...
    if(recv.size() != _read_len) recv.resize(_read_len);
    _recv_copy = recv;
    
    //------------------------------
    // この部分を削除することで，データを受信できなかった時にエラーの値-1024ではなく前回受信した値が出力されます．
    _measurement.lat = _measurement.lon = _measurement.target_angle = _measurement.target_distance = -1024.0;
    _measurement.altitude = _measurement.HDOP = _measurement.geoid_separation = _measurement.velocity = _measurement.second = -1024.0F;
    _measurement.minute = _measurement.hour = _measurement.day = _measurement.month = _measurement.year = -1024;
    //------------------------------
    ...
    while(i < int(_read_len)) {
        ...
        while(i < int(_read_len) && _recv_copy.at(i++)!='$');  //最初の$まで移動
        for(; i < int(_read_len) && _recv_copy.at(i)!='*'; i++){ //*になるまで次の文字を見ていく
        ...
        if(i >= int(_read_len)) break;

        std::string check_sum_str = "";

        i++;
        while(i < int(_read_len) && _recv_copy.at(i)!='\r' && _recv_copy.at(i)!='\n'){
...
inline void GPS::output_time(std::string &value_str){
    if(value_str.size() == 9){
        _measurement.hour = ((value_str.at(0)-'0')*10 + (value_str.at(1)-'0') + time_diff) % 24;
        _measurement.minute = (value_str.at(2)-'0')*10 + value_str.at(3)-'0';
        _measurement.second = (value_str.at(4)-'0')*10 + (value_str.at(5)-'0'); //小数形式で与えられるが，.00しか出力されないようなので小数部分は無視する
        printf("gps_time: %d:%d:%d\n", _measurement.hour, _measurement.minute, _measurement.second);
    }
}
...
inline void GPS::output_lat(std::string &value1_str, std::string &value2_str){
    if(value1_str.size() == 10 && value2_str != ""){    
        _measurement.lat = ((value1_str.at(0)-'0')*10 + (value1_str.at(1)-'0') + ((value1_str.at(2)-'0')*10 + (value1_str.at(3)-'0') + (value1_str.at(5)-'0')*0.1 + (value1_str.at(6)-'0')*0.01 + (value1_str.at(7)-'0')*0.001 + (value1_str.at(8)-'0')*0.0001 + (value1_str.at(9)-'0')*0.00001)/60.0) * ((value2_str=="N")?1:-1);
        printf("gps_lat: %f\n", _measurement.lat);
    }
}
...
inline void GPS::output_lon(std::string &value1_str, std::string &value2_str){
    if(value1_str.size() == 11 && value2_str != ""){    
        _measurement.lon = ((value1_str.at(0)-'0')*100 + (value1_str.at(1)-'0')*10 + (value1_str.at(2)-'0') + ((value1_str.at(3)-'0')*10 + (value1_str.at(4)-'0') + (value1_str.at(6)-'0')*0.1 + (value1_str.at(7)-'0')*0.01 + (value1_str.at(8)-'0')*0.001 + (value1_str.at(9)-'0')*0.0001 + (value1_str.at(10)-'0')*0.00001)/60.0) * ((value2_str=="E")?1:-1);
        printf("gps_lon: %f\n", _measurement.lon);
    }
}
...
inline void GPS::output_date(std::string &value_str){
    if(value_str.size() == 6){
        ...
        _measurement.year = 2000 + (value_str.at(4)-'0')*10 + (value_str.at(5)-'0');
        printf("gps_date: %d-%d-%d\n", _measurement.day, _measurement.month, _measurement.year);
    }
}

//目標物との角度と距離を出力するための変数に入れる
void GPS::output_target(){
    if(_measurement.lat==-1024.0 || _measurement.lon==-1024.0) return;
    ...
    //同じ二点では角度は定義されない
    if(_measurement.lon==_target_lon && _measurement.lat==_target_lat) return;
    
    _measurement.target_angle = atan((cos(m_lat_rad)*cos(t_lat_rad)*sin(t_lon_rad-m_lon_rad))/(sin(t_lat_rad)-sin(m_lat_rad)*(cos_n))) * 180 / M_PI;
    //東経から西経などへの計算も正確にするため，場合分けをしている．
    if     (_target_lon-_measurement.lon > 180) _measurement.target_angle += (_measurement.target_angle < 0 ? 360 : 180);
    else if(_target_lon-_measurement.lon >   0) _measurement.target_angle += (_measurement.target_angle < 0 ? 180 :   0);
    else if(_target_lon-_measurement.lon >-180) _measurement.target_angle += (_measurement.target_angle < 0 ? 360 : 180);
    else                                      _measurement.target_angle += (_measurement.target_angle < 0 ? 180 :   0);
}
        ...
        if(_measurement.hour < time_diff){
            if(_measurement.day >= 28){
                ...
                if(_measurement.day <= 29){
                    ...
                    if(_measurement.month==4 || _measurement.month==6 || _measurement.month==9 || _measurement.month==11){
                        _measurement.month++;
                        _measurement.day = 1;
                    }else{
                        _measurement.day++;
                    ...
                    if(_measurement.month==12){
                        if(_measurement.year!=-1024) _measurement.year++;
