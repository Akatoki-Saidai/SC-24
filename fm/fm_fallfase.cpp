...
std::pair<double, double> calc_xy(double phi_deg, double lambda_deg, double phi0_deg, double lambda0_deg) {
    ...
    auto alpha_array = [](double n) {
        std::array<double, 6> alpha = {
            std::nan(""),
            (1. / 2) * n - (2. / 3) * std::pow(n, 2) + (5. / 16) * std::pow(n, 3) + (41. / 180) * std::pow(n, 4) - (127. / 288) * std::pow(n, 5),
            (13. / 48) * std::pow(n, 2) - (3. / 5) * std::pow(n, 3) + (557. / 1440) * std::pow(n, 4) + (281. / 630) * std::pow(n, 5),
            (61. / 240) * std::pow(n, 3) - (103. / 140) * std::pow(n, 4) + (15061. / 26880) * std::pow(n, 5),
...
void fallfase()
{
    ...
    double goal_angle_cansat_basis = goal_angle_cansat_basis + M_PI;
    //角度から指示を出す。
...
//     double a0 = std::nan(""); // dummy,vectorの長さを6にするために置いてるらしい...おいらにはよくわからない
//     double a1 = (1.0/2.0)*n - (2.0/3.0)*(n*n) + (5.0/16.0)*(n*n*n) + (41.0/180.0)*(n*n*n*n) - (127.0/288.0)*(n*n*n*n*n);
//     double a2 = (13.0/48.0)*(n*n) - (3.0/5.0)*(n*n*n) + (557.0/1440.0)*(n*n*n*n) + (281.0/630.0)*(n*n*n*n*n);
//     double a3 = (61.0/240.0)*(n*n*n) - (103.0/140.0)*(n*n*n*n) + (15061.0/26880.0)*(n*n*n*n*n);
//     double a4 = (49561.0/161280.0)*(n*n*n*n) - (179.0/168.0)*(n*n*n*n*n);
//     double a5 = (34729.0/80640.0)*(n*n*n*n*n);
    
//     return {a0, a1, a2, a3, a4, a5};
// }

// // 2次元のベクトルをxy平面で時計回りに回転する関数
// std::vector<double> Rotation_clockwise_xy(std::vector<double> vec, double Radian) {
...
// //距離を求める関数
// std::vector<double> calc_xy(double phi_deg,double lambda_deg,double phi0_deg,double lambda0_deg){
//     //  緯度経度を平面直角座標に変換する
//     // - input:
...
//     std::vector<double> array_sin = {std::sin(2*phi0_rad*1),std::sin(2*phi0_rad*2),std::sin(2*phi0_rad*3),std::sin(2*phi0_rad*4),std::sin(2*phi0_rad*5)};//chu!冗長でごめん!
//     double dot_array5_arraysin = std::inner_product(A_array_5,array_sin);
//     double S_ = ( (m0*a)/(1.+n) )*( A_array_6[0]*phi0_rad + dot_array5_arraysin ); // [m]
